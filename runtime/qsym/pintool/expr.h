...
class Expr : public DependencyNode {
  public:
    ...
    bool isBit() const {
      return (kind_ <= qsym::AShr && kind_ >= qsym::Not);
    }

    bool isCmp() const{
      return (kind_ <= qsym::FUne && kind_ >= qsym::FOgt) ||
             (kind_ <= qsym::Sge && kind_ >= qsym::Equal);
    }

    bool isLogic() const{
      return (kind_ <= qsym::LNot && kind_ >= qsym::LOr);
    }
...
public:
  ...
  bool ford(ConstantFloatExpr &RHS) {
    return value_.compare(RHS.value()) != llvm::APFloat::cmpUnordered;
  }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    if (bits_ == 32)
      return context_.fpa_val(value_.convertToFloat());
    else
      return context_.fpa_val(value_.convertToDouble());
  }

  bool printAux(ostream &os) const override {
    os << "value=0x" << LLVMIntToString(value_.bitcastToAPInt(), 16)
       << ", bits=" << bits_;
    return true;
  }
...
class NonConstantExpr : public Expr {
  public:
    ...
    static bool classOf(const Expr& e) {
      return !ConstantExpr::classOf(e) && !ConstantFloatExpr::classOf(e);
    }
};
...
class FPToBVExpr : public UnaryExpr{
public:
  FPToBVExpr(ExprRef e)
      : UnaryExpr(FPToBV, e,e->bits()){
  }
  static bool classOf(const Expr& e) { return e.kind() == FPToBV; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    return children_[0]->toZ3Expr(verbose).mk_to_ieee_bv();
  }
};

class BVToFPExpr : public UnaryExpr{
public:
  BVToFPExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(BVToFP, e, bits){
  }
  static bool classOf(const Expr& e) { return e.kind() == BVToFP; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    if (bits_ == 32){
      z3::sort sort = context_.fpa_sort(8, 24);
      return children_[0]->toZ3Expr(verbose).mk_from_ieee_bv(sort);
    }else{
      z3::sort sort = context_.fpa_sort(11, 53);
      return children_[0]->toZ3Expr(verbose).mk_from_ieee_bv(sort);
    }
  }
};

class FPToFPExpr : public UnaryExpr {
public:
  FPToFPExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(FPToFP, e, bits) {
  }

  static bool classOf(const Expr &e) { return e.kind() == FPToFP; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    if (bits_ == 32){
      z3::sort sort = context_.fpa_sort(8, 24);
      return z3::fpa_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }else{
      z3::sort sort = context_.fpa_sort(11, 53);
      return z3::fpa_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }
...
class FPToSIExpr : public UnaryExpr {
public:
  FPToSIExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(FPToSI, e, bits) {
  }

  static bool classOf(const Expr &e) { return e.kind() == FPToSI; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    return z3::fpa_to_sbv(children_[0]->toZ3Expr(verbose), bits_);
  }
};

class FPToUIExpr : public UnaryExpr {
public:
  FPToUIExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(FPToUI, e, bits) {
  }

  static bool classOf(const Expr &e) { return e.kind() == FPToUI; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    return z3::fpa_to_ubv(children_[0]->toZ3Expr(verbose), bits_);
  }
};

class SIToFPExpr : public UnaryExpr {
public:
  SIToFPExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(SIToFP, e, bits) {
  }

  static bool classOf(const Expr &e) { return e.kind() == SIToFP; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    if (bits_ == 32){
      z3::sort sort = context_.fpa_sort(8, 24);
      return z3::sbv_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }else{
      z3::sort sort = context_.fpa_sort(11, 53);
      return z3::sbv_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }
...
class UIToFPExpr : public UnaryExpr {
public:
  UIToFPExpr(ExprRef e, UINT32 bits)
      : UnaryExpr(UIToFP, e, bits) {
  }

  static bool classOf(const Expr &e) { return e.kind() == UIToFP; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    if (bits_ == 32){
      z3::sort sort = context_.fpa_sort(8, 24);
      return z3::ubv_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }else{
      z3::sort sort = context_.fpa_sort(11, 53);
      return z3::ubv_to_fpa(children_[0]->toZ3Expr(verbose), sort);
    }
...
public:
  ...
  inline bool isFalse() const { return !value_; }
...
class FAddExpr : public LinearBinaryExpr {
public:
  FAddExpr(ExprRef l, ExprRef h)
      : LinearBinaryExpr(FAdd, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FAdd; }
  void print(ostream &os, UINT depth) const override;
...
class FSubExpr : public LinearBinaryExpr {
public:
  FSubExpr(ExprRef l, ExprRef h)
      : LinearBinaryExpr(FSub, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FSub; }
  void print(ostream &os = std::cerr, UINT depth = 0) const override;
protected:
  std::string getName() const override {
    return "FSub";
...
class FMulExpr : public NonLinearBinaryExpr {
public:
  FMulExpr(ExprRef l, ExprRef h)
      : NonLinearBinaryExpr(FMul, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FMul; }
  void print(ostream &os, UINT depth) const override;
...
class FDivExpr : public NonLinearBinaryExpr {
public:
  FDivExpr(ExprRef l, ExprRef h)
      : NonLinearBinaryExpr(FDiv, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FDiv; }
  void print(ostream &os, UINT depth) const override;
...
class FAbsExpr : public UnaryExpr {
public:
  FAbsExpr(ExprRef e)
      : UnaryExpr(FAbs, e) {}

  static bool classOf(const Expr &e) { return e.kind() == FAbs; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    return z3::abs(children_[0]->toZ3Expr(verbose));
  }
};

class FOgtExpr : public CompareExpr {
public:
  FOgtExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOgt, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOgt; }
...
class FOgeExpr : public CompareExpr {
public:
  FOgeExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOge, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOge; }
...
class FOltExpr : public CompareExpr {
public:
  FOltExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOlt, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOlt; }
...
class FOleExpr : public CompareExpr {
public:
  FOleExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOle, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOle; }
...
class FOeqExpr : public CompareExpr {
public:
  FOeqExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOeq, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOeq; }
...
class FOneExpr : public CompareExpr {
public:
  FOneExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOne, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOne; }
...
class FRemExpr : public NonLinearBinaryExpr {
public:
  FRemExpr(ExprRef l, ExprRef h)
      : NonLinearBinaryExpr(FRem, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FRem; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    return z3::rem(children_[0]->toZ3Expr(verbose),
                    children_[1]->toZ3Expr(verbose));
  }
};

class FOrdExpr : public CompareExpr {
public:
  FOrdExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOrd, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FOrd; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    z3::expr expr0 = children_[0]->toZ3Expr(verbose).mk_is_nan();
    z3::expr expr1 = children_[0]->toZ3Expr(verbose).mk_is_nan();
    return expr0 || expr1;
  }
};

class FUnoExpr : public CompareExpr {
public:
  FUnoExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUno, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUno; }

protected:
  std::string getName() const override {
    return "FUno";
  }
  // a 或 b 是 NaN, 就返回 true
  z3::expr toZ3ExprRecursively(bool verbose) override {
    z3::expr expr0 = children_[0]->toZ3Expr(verbose).mk_is_nan();
    z3::expr expr1 = children_[0]->toZ3Expr(verbose).mk_is_nan();
    z3::expr check = expr0 || expr1;
    return check;
  }
};

class FUgtExpr : public CompareExpr {
public:
  FUgtExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUgt, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUgt; }
...
protected:
  ...
  z3::expr toZ3ExprRecursively(bool verbose) override {
    z3::expr expr0 = children_[0]->toZ3Expr(verbose);
    z3::expr expr1 = children_[0]->toZ3Expr(verbose);
    return expr0.mk_is_nan() || expr1.mk_is_nan() || expr0 > expr1;
  }
};

class FUgeExpr : public CompareExpr {
public:
  FUgeExpr(ExprRef l, ExprRef h)
      : CompareExpr(FOge, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUge; }
...
class FUltExpr : public CompareExpr {
public:
  FUltExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUlt, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUlt; }
...
class FUleExpr : public CompareExpr {
public:
  FUleExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUle, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUle; }
...
class FUeqExpr : public CompareExpr {
public:
  FUeqExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUeq, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUeq; }
...
class FUneExpr : public CompareExpr {
public:
  FUneExpr(ExprRef l, ExprRef h)
      : CompareExpr(FUne, l, h) {}

  static bool classOf(const Expr &e) { return e.kind() == FUne; }
